  ...
  tasks:
    ...
    - name: copy the sql script
      ansible.builtin.copy:
        src: /home/ubuntu/phonebook/init.sql
        dest: ~/
...
- name: web server configuration
  ...
  tasks:
    ...
    - name: copy the templates folder to the web server
      ansible.builtin.copy:
        src: /home/ubuntu/phonebook/templates
        dest: ~/
...
- Now, we create one more EC2 instance and add it to our inventory. (``amazon linux 2023, t3.small``)
...
- hosts: servers
 ...
 # ignore_errors: true  # for all task
...
# our intention is to make node3 fail in the third task.
    - name: task 3
      ...
      ansible.builtin.apt:
        name: git
        state: present
   #   ignore_errors: true  # only for this task
