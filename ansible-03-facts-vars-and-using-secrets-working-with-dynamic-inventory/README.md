...
- Explain how and what fact gathering is and how to use it in the playbook
- Learn how to deal with secret values with Ansible Vault
- Explain what a dynamic inventory is
...
- Notice that in the playbook, we use the same content for each node, but with a different IP address. This time, we get the ip address value from a variable. Update the myplaybook.yaml as follows.
...
- Create another encrypted file using the "ansible-vault" command, but this time use SHA (Secure Hash Algorithm) for your password:
...
- Celect actions -> security -> modify IAM role
...
- To ensure that all our hosts are reachable with dynamic inventory, we will run various ad-hoc commands using the ping module.
...
- Create an encrypted file using the "ansible-vault" command. Use `123456` as a password.
